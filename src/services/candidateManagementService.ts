...
export class CandidateManagementService {
  ...
  async getCandidates(page: number = 1, limit: number = 10, search: string = '') {
    try {
      ...
      const [countResult] = await mysqlPool.execute(countQuery, params);
      ...
      const [candidates] = await mysqlPool.execute(query, [...params, limit, offset]);
  ...
  async getCandidateById(id: number) {
    try {
      ...
      `;
      const [candidateResult] = await mysqlPool.execute(candidateQuery, [id]);
      const candidate = (candidateResult as any)[0];
      ...
      `;
      const [educationResult] = await mysqlPool.execute(educationQuery, [id]);
      ...
      const workExperienceQuery = `
        ...
        ORDER BY id ASC
      `;
      const [workExperienceResult] = await mysqlPool.execute(workExperienceQuery, [id]);
      ...
      `;
      const [skillsResult] = await mysqlPool.execute(skillsQuery, [id]);
      ...
      `;
      const [preferencesResult] = await mysqlPool.execute(preferencesQuery, [id]);
      candidate.job_preferences = (preferencesResult as any)[0] || null;
    ...
    try {
      ...
      try {
        ...
        if (data.preferences) {
          ...
          const [jpCheck] = await connection.execute(
            'SELECT id FROM candidate_preferences WHERE candidate_id = ?',
            [id]
          );
          
          if ((jpCheck as any).length > 0) {
            // Update existing
            const jpUpdateQuery = `
  ...
  async exportToCSV(search: string = '') {
    try {
      // Build WHERE clause
      let whereClause = 'WHERE 1=1';
      const params: any[] = [];
      ...
      const [candidates] = await mysqlPool.execute(query, params);
      ...
      const csvRows = (candidates as any[]).map(candidate => [
        candidate.id,
        candidate.full_name,
        candidate.email,
